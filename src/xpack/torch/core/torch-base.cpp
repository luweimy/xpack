...
std::string torch::ToHex(const char *data, size_t length, int byteblank)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        osstream<< hextable[(uc>>4)] << hextable[(uc&0x0f)];
        if ( byteblank>0 && ((i+1)%byteblank == 0) && (i+1)<length ) {
            osstream<<(' ');
        }
    }
    return osstream.str();
...
std::string torch::ToBinary(const char *data, size_t length, int byteblank)
{
    std::ostringstream osstream;
...
void torch::DumpHex(const char *data, size_t length, int byteblank)
{
    printf("%s\n", torch::ToHex(data, length, byteblank).c_str());
}

void torch::DumpBinary(const char *data, size_t length, int byteblank)
{
    printf("%s\n", torch::ToBinary(data, length, byteblank).c_str());
}
