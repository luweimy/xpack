...
Data& Data::Fill(int index, size_t size, char ch)
{
    if (this->GetSize() < index + size) {
        m_allocator.ReAlloc(index + size);
    }
    char *ptr = (char*)this->GetBytes() + index;
    memset(ptr, ch, size);
    return *this;
}

Data& Data::Fill(int index, char ch)
{
    this->Fill(index, 1, ch);
    return *this;
}

Data& Data::Fill(char ch)
{
    this->Fill(0, this->GetSize(), ch);
    return *this;
}
...
std::string Data::ToHex(int byteblank) const
{
    return torch::ToHex((char*)m_allocator.GetPtr(), m_allocator.GetSize(), byteblank);
}

std::string Data::ToBinary(int byteblank) const
{
    return torch::ToBinary((char*)m_allocator.GetPtr(), m_allocator.GetSize(), byteblank);
}

void Data::DumpHex(int byteblank) const
{
    torch::DumpHex((char*)m_allocator.GetPtr(), m_allocator.GetSize(), byteblank);
}

void Data::DumpBinary(int byteblank) const
{
    torch::DumpBinary((char*)m_allocator.GetPtr(), m_allocator.GetSize(), byteblank);
}
