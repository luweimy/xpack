...
namespace torch { namespace crypto {
    ...
    public:
         ...
         */
        RC4& SetSecretKey(const unsigned char *skey, const int length);
         ...
         *  - 加密和解密都调用同一个接口
         */
        Data CryptoCopy(const unsigned char *input, const int length);
         ...
         *  - 加密和解密都调用同一个接口
         */
        void CryptoNoCopy(unsigned char *input, int length);
         ...
         *  - 接口若发生错误，则返回空的Data对象
         */
        Data CryptoByFile(const std::string &path);
