...
namespace torch { namespace crypto {
    ...
    public:
         ...
         */
        Crc32();
        void ComputeBlock(const unsigned char *input, const int length);
         ...
         * 注意：
         *  - 接口[ComputeByFile()]若发生错误，则返回0
         */
        static uint32_t Compute(const unsigned char *input, const int length);
        static uint32_t Compute(const Data &input);
        static uint32_t ComputeByFile(const std::string &path);
