...
RC4& RC4::SetSecretKey(const unsigned char *skey, const int length)
{
    m_skey.CopyFrom(skey, length);
    return *this;
}

torch::Data RC4::CryptoCopy(const unsigned char *input, const int length)
{
...
void RC4::CryptoNoCopy(unsigned char *input, int length)
{
    arc4_setup(&m_rc4ctx, (unsigned char *)m_skey.GetBytes(), (int)m_skey.GetSize());
    arc4_crypt(&m_rc4ctx, input, length);
}
...
torch::Data RC4::CryptoByFile(const std::string &path)
{
